...
class PluginManagerSpec extends BaseSpec {
  ...
  val classPackage1: String = PluginManagerSpecCompanion.makeFullyQualifiedName(packagePrefix, "MockPlugin1")
  val classPackage2: String = PluginManagerSpecCompanion.makeFullyQualifiedName(packagePrefix, "MockPlugin2")
  ...
  "ActivePlugins" should {
    ...
    "run" should {
      ...
      "return a failure" in {
        ...
        val pluginManager: PluginManager = new PluginManager(Seq(mockPluginBase))
        intercept[StatusException](pluginManager.run(processCommandRequest, metadata).get)
      }
    }
  }
}
...
object PluginManagerSpecCompanion extends Matchers {
   ...
   */
  def makeFullyQualifiedName(prefix: String, suffix: String): String = s"$prefix.$suffix"
}
